...
4. **参数优先级**：
   - **首次创建**：序列首次使用时，会使用提供的所有参数（name, padding, start, step）创建序列
   - **后续使用**：序列已存在时，会忽略 `start` 和 `step` 参数，使用序列配置中的值
   - **padding 特殊处理**：`padding` 参数仅用于格式化显示，不会修改序列配置。这意味着：
     - 在 Request 1 中使用 `{{$sequence(padding=4)}}`，显示为 `0001`
     - 在 Request 2 中使用 `{{$sequence(padding=2)}}`，显示为 `02`
     - 两者使用同一个序列，只是显示格式不同
5. **避免参数冲突**：
   - 如果多个请求使用同一个序列名称，建议在序列管理界面统一配置
   - 请求中的参数仅在首次创建时生效，后续使用会被忽略
   - 如需修改序列的核心配置（step, startValue），请使用序列管理界面
6. **修改参数**：如需修改序列参数，请使用序列管理界面
7. **重置序列**：通过序列管理界面可以将序列重置到任意值
...
**Request 1**：
```
{{$sequence(name=order, padding=4, start=100, step=2)}}
```

**Request 2**：
```
{{$sequence(name=order, padding=6, start=200, step=5)}}
```

**实际行为**：
- 第一次发送 Request 1：创建序列 `order`，配置为 `start=100, step=2, padding=4`，返回 `0100`
- 第二次发送 Request 1：使用已存在的序列，返回 `0102`（100 + 2）
- 第一次发送 Request 2：使用已存在的序列 `order`，**忽略** `start=200, step=5` 参数，但使用 `padding=6` 格式化，返回 `000104`（102 + 2）
- 第二次发送 Request 2：返回 `000106`（104 + 2）

**关键点**：
- 序列的 `step` 始终是 `2`（首次创建时的值）
- `padding` 参数仅影响显示格式，Request 1 显示 4 位，Request 2 显示 6 位
- `start` 参数只在首次创建时生效

**最佳实践**：
1. 在序列管理界面统一配置序列的核心参数（step, startValue）
2. 在请求中只使用 `name` 和 `padding` 参数
3. 如果需要不同的 step，使用不同的序列名称
